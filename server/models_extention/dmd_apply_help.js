...
const dmd_apply_help = {
    ...
    checkApply(money, member) {
        return co(function*() {
            const conf22 = yield models.dmd_config.getConfig(22) //系统是否允许收获
            ...
            const conf32 = yield models.dmd_config.getConfig(32)
            const conf32list = conf32.val.split('-')
            ...
            const conf3 = yield models.dmd_config.getConfig(3) //申请帮助最小金额 - 被整除数
            const conf3list = conf3.val.split('-')
            ...
            const conf8 = yield models.dmd_config.getConfig(8) //收获每天最高限额（单位：元）
            ...
            if (alreadyApplyMoney > money + Number(conf8.val)) {
